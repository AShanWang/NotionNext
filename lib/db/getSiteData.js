...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  const defaultTitle = NOTION_CONFIG?.TITLE || '阿山的藏宝图'
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '试炼区-试水1号（半成品）'
  const defaultPageCover = NOTION_CONFIG?.HOME_BANNER_IMAGE || '/bg_image.jpg'
  const defaultIcon = NOTION_CONFIG?.AVATAR || 'https://img.gugu.ovh/i/2023/07/28/vknqn.webp'
